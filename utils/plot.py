...
path ="/network/home/jainarus/OC/a2oc_delib/images"
plt.ion()
fig = plt.figure()
...
while True:
  try:
    ...
    for dd in sys.argv[1:]:
      if "--" not in dd:
        legend = dd.split("/")[-1]
        tempname = legend.split("_EPS0.2_")[0]
        psivalue = float(tempname.split("_PSI")[1])
        name_game = tempname.split("_")[0]
	if psivalue!=0:
		tname = " (Safe-A2OC)"
	else:
		tname = " (A2OC)"
    ...
    plt.draw()
    plt.savefig((os.path.join(path,name_game+ ".png")), dpi=200, facecolor='w', edgecolor='w',
